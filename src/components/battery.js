...
export default class notifList extends React.Component {
  ...
  deleteNotification(id,e) {
    console.log(id);
    axios.delete(`https://ifttt-mobile-app.herokuapp.com/battery/${id}`).then(res => {
      axios.get(`https://ifttt-mobile-app.herokuapp.com/battery`).then(res => {
    ...
    return this.state.notifications.map(notf => (
      <div key={notf._id} className="item">
        <ul className="list-group">
          <li className="list-group-item active">{notf.powerSource}</li>
          <li className="list-group-item ">{notf.batteryPercentage}%</li>
